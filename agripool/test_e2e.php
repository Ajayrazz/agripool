...
// Helper to get token via artisan
function getToken($role) {
    $script = "<?php echo App\Models\User::where('role', '$role')->first()->createToken('test')->plainTextToken;";
    file_put_contents('test_token.php', $script);
    $output = shell_exec('php artisan tinker test_token.php 2>/dev/null');
    return trim(str_replace('>', '', strip_tags($output)));
}

$transporterToken = getToken('transporter');
$farmerToken = getToken('farmer');

echo "Transporter Token: " . substr($transporterToken, 0, 10) . "...\n";
echo "Farmer Token: " . substr($farmerToken, 0, 10) . "...\n";

// Step 1: Transporter schedules a route
echo "\n=== Step 1: Transporter ===\n";
...
// Check database for capacity decrease
echo "\n=== Checking Remaining Capacity ===\n";
$capacityScript = "<?php echo App\Models\Vehicle::find($vehicleId)->remaining_capacity_kg;";
file_put_contents('test_cap.php', $capacityScript);
$capacity = trim(str_replace('>', '', strip_tags(shell_exec('php artisan tinker test_cap.php 2>/dev/null'))));
if ($capacity == 9500) echo "PASS: Remaining capacity decreased to 9500 kg.\n";
else echo "FAIL: Remaining capacity is $capacity (expected 9500 kg).\n";
